...
abstract class Action
{
    ...
    final public function configureAtomicLock(AtomicOptions $shouldLock): self
    {
    ...
    final public function withoutLock(): self
    {
    ...
    final public function handle(...$args)
    {
        ...
        if (!$this->shouldLock) {
            return $this->{self::EXECUTION_HANDLER}(...$args);
        }

        return cache()
            ->lock($this->generateAtomicLockId(...$args), $this->lockTtl)
            ->block($this->lockWaitSeconds, fn() => $this->{self::EXECUTION_HANDLER}(...$args));
    }
